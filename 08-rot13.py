...
for i in range(???,???):  #Interate alphabets and assign key value pair. Value is i+13 th value.
    key[???] = alphabet[???%26]                  # Assigning 13th alphabet from current one as key value pair 
    key[???] = key[???].???     # Also assign upper case letters.
...
for x in ???:                                         # Loop to iterate and replace characters
    if x in ???:                                            # from the key
        decrypted=???+key[???]
    else:
        decrypted=???+???
