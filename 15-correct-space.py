...
def correct(???):                    # creating a function to correct the input string
    dot=re.sub(r'\.','. ',???)       # regular expression to add a space after a dor (.) wherever required
    sq=re.sub(???'???','???',???)            # regular expression to remove extra spaces after a single space has occured
    return ???                           # returining final corrected string
   
strng="Do not     go gentle into that    good night.Rage,rage  against  the   dying of light. Goodbye!"
print(???(???))                   # printing correct string